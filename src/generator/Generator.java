/**
 * I have no idea how it works. Better now touch it.
 */
package generator;
...
/**
 * Class for creating new levels.
 * 
 * @author tutor (edited for own use: mike239x)
 */
public class Generator {
    ...
    static int[] nxt(int x, int y) {
        ...
        if (x == 0) {
            ...
            if (y == width - 2) {
                return bad ? nxt(height - 2, width - 1) : new int[]{height - 2, width - 1};
            }
            return bad ? nxt(x, y + 1) : new int[]{x, y + 1};
        } else if (y == 0) {
            if (x == height - 2) {
                return bad ? nxt(height - 1, 1) : new int[]{height - 1, 1};
            }
            return bad ? nxt(x + 1, y) : new int[]{x + 1, y};
        } else {
            if (x == 1) {
                return bad ? nxt(0, width - 2) : new int[]{0, width - 2};
            }
            return bad ? nxt(x - 1, y) : new int[]{x - 1, y};
        }
    }
    ...
    static long init() throws Exception {
        ...
        while (idx < points.length) {
            do {
                points[idx] = new int[]{modnar(1, height - 2), modnar(1, width - 2)};
    ...
    static int modnar(int low, int high) {
        ...
        return Math.abs(random.nextInt()) % (1 + high - low) + low;
    }

    static boolean chance(int percentage) {
        return Math.abs(random.nextInt()) % 100 < percentage;
    }
    ...
    static int[] connectionPoint(int x, int y) {
        for (int dist = 1; dist < 10000; dist++) {
            for (int xx = x - dist < 1 ? 1 : x - dist; xx < height - 1 && xx - x < dist + 1; xx++) {
                if (y + dist < width && occupied[xx][y + dist]) {
                    int[] res = {xx, y + dist};
                    return res;
                }
            ...
            for (int yy = y - dist < 1 ? 1 : y - dist; yy < width - 1 && yy - y < dist + 1; yy++) {
                if (x + dist < height && occupied[x + dist][yy]) {
                    int[] res = {x + dist, yy};
                    return res;
                }
    ...
    static void connect(int x1, int y1, int x2, int y2) {
        ...
        if (choice == 1) {
            if (chance(50)) {
                ...
                if (x1 < x2 == y1 < y2) {
                    diry(maxX, minY, maxY);
                    if (field[maxX][minY] == wall) {
                        field[maxX][minY] = free;
                    }
                } else {
                    diry(minX, minY, maxY);
                    if (field[minX][minY] == wall) {
                        field[minX][minY] = free;
                    }
                }
            } else {
                dirx(maxY, minX, maxX);
                if (x1 < x2 == y1 < y2) {
                    diry(minX, minY, maxY);
                    if (field[minX][maxY] == wall) {
                        field[minX][maxY] = free;
                    }
                } else {
                    diry(maxX, minY, maxY);
                    if (field[maxX][maxY] == wall) {
                        field[maxX][maxY] = free;
                    }
                }
            }
    ...
    static void read() {
        ...
        if (s != null) {
            density = Double.valueOf(s).doubleValue();
        }
    }

    @SuppressWarnings("CallToPrintStackTrace")
    public static Properties generate(Properties settings) {
        printField = false;
        param = settings;
        try {
