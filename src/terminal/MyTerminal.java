/**
 * Copy of lanterna swing terminal, but unresizable. I also removed a 
 * strange line on the right of the screen (maybe it was also me who
 * created it, but who cares).
 * This file was part of lanterna (http://code.google.com/p/lanterna/).
...
/**
 * A Swing-based text terminal emulator, cannot be resized.
 *
 * @author Martin
 * + little change by mike239x
 */
public class MyTerminal extends AbstractTerminal implements InputProvider
{    
    ...
    public MyTerminal(TerminalSize terminalSize) 
    {
        this(terminalSize.getColumns(), terminalSize.getRows());
    }
    
    public MyTerminal(int widthInColumns, int heightInRows)
    {
        this(TerminalAppearance.DEFAULT_APPEARANCE, widthInColumns, heightInRows);
    }
    
    public MyTerminal(TerminalAppearance appearance)
    {
        this(appearance, 160, 40);
    }
    
    public MyTerminal(TerminalAppearance appearance, int widthInColumns, int heightInRows)
    {
     ...
     * 
     * @return JFrame object used by this MyTerminal
     */
    ...
    public void enterPrivateMode() {
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                terminalFrame = new JFrame("CapStone v.1.1.");
                ...
                terminalFrame.pack();
                //some strange stuff to remove the empty line
                //on the left of the screen.
                int w = getTerminalSize().getColumns();
                terminalFrame.setSize(
                        terminalFrame.getWidth() * w / (w + 1),
    ...
    private class TerminalRenderer extends JComponent
    {
        ...
        public Dimension getPreferredSize()
        {
            ...
            final int screenWidth = MyTerminal.this.size().getColumns() * fontMetrics.charWidth(' ');
            final int screenHeight = MyTerminal.this.size().getRows() * fontMetrics.getHeight();
        ...
        protected void paintComponent(Graphics g)
        {
            ...
            for(int row = 0; row < MyTerminal.this.size().getRows(); row++) {
                for(int col = 0; col < MyTerminal.this.size().getColumns(); col++) {
